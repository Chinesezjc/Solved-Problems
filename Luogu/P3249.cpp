        ...
        for (int j = 0; j != (int)a[i].to.size(); ++j)
            if (!a[i].to[j].tob)
            {
                ...
                do
                {
                    ...
                    // std::cout << nowp << ' ' << a[nowp].to[nowe].to << std::endl;
                    if (a[nxtp].to[nxte].tob)
                    {
                        // std::cout << "connect " << a[nowp].to[nowe].tob << ' ' << a[nxtp].to[nxte].tob << std::endl;
...
#ifdef debug
            if (!a[lst].to[e].p)
                std::cout << a[lst].to[e].p << std::endl;
            if (!in)
                std::cout << now << ' ' << (xors[a[lst].to[e].p] ^ e) << std::endl;
            if (!out)
                std::cout << lst << ' ' << e << std::endl;
            std::cout << "lst now:" << lst << ' ' << now << std::endl;
            std::cout << "in out:" << in << ' ' << out << std::endl;
