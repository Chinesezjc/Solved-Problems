...
struct High_Accuracy_Algorithm
{
	...
	inline High_Accuracy_Algorithm operator =(string a)
	{
		n=0;
		int len=a.length();
		if(a[0]=='-')
...
inline int logarithm(const High_Accuracy_Algorithm x,const High_Accuracy_Algorithm index)//wrong
{
	if(x==1||x==-1)
	{
		exit(114514);
	}
	High_Accuracy_Algorithm l[27],sum,nxt;
	sum="1";
	int tmp=0,cnt=0;
	l[0]=x;
	while((l[cnt].n-1)<<1<=x.n)
	{
		++cnt;
		l[cnt]=l[cnt-1]*l[cnt-1];
	}
	for(int i=cnt;i>=0;--i)
	{
		nxt=sum*l[i];
