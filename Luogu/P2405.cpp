...
int n, p[10005], cnt, small, big;
zjcSTL::bigint<100000000> m;
signed main()
{
    ...
    while (!m.empty())
    {
        p[cnt++] = atoi(((string)(m % n)).c_str());
        m /= n;
    }
    big = 1;
    while (cnt--)
    {
        int lsts = small, lstb = big;
        small = min(lsts + p[cnt], lstb + n - p[cnt]);
        big = min(lsts + p[cnt] + 1, lstb + n - p[cnt] - 1);
    }
    cout << small << endl;
    return 0;
}