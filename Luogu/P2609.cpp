...
int T;
zjcSTL::bigint<100000000> n;
map<zjcSTL::bigint<100000000>, zjcSTL::bigint<1000000000>> g;
signed main()
{
    ...
    while (T--)
    {
        ...
        g.clear();
        g[n] = 1;
        while (g.rbegin()->first > 1)
        {
            g[g.rbegin()->first / 2] += g.rbegin()->second;
            if (g.rbegin()->first % 2 == 1)
            {
                g[g.rbegin()->first / 2 + 1] += g.rbegin()->second;
            }
            g.erase(--g.end());
        }
        cout << g[1] << endl;
