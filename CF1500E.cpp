...
#define mid ((l + r) / 2)
    ...
    int QueryL(int w, int l, int r, int x)
    {
        if (l == r)
            return l;
        if (t[ls].tot >= x)
            return QueryL(ls, l, mid, x);
        else
            return QueryL(rs, mid + 1, r, x - t[ls].tot);
    }
...
int solve()
{
    if (num <= 1)
        return 0;
    if (num == 2)
        return t.QueryRkth(1) - t.QueryLkth(1);
    int x = (num - 1) / 2;
    int l = 1, r = x;
    while (l <= r)
    {
...
signed main()
{
    ...
    for (int i = 1; i <= m; i++)
        q[i].x = lower_bound(b + 1, b + 1 + cnt, q[i].x) - b;
    for (int i = 1; i <= n; i++)
        t.add(1, a[i]);
    print(solve());
    for (int i = 1; i <= m; i++)
